...
void WorldSystem::on_key(int key, int, int action, int mod) {
	...
	if ((action == GLFW_REPEAT || action == GLFW_PRESS) && key == GLFW_KEY_UP) {
		for (Entity entity : registry.players.entities) {
			...
			motion.velocity[1] = sin(angle) * -velocity;
			motion.velocity[0] = cos(angle) * -velocity;
			registry.slidings.remove(entity);
		}
	}
	...
	if (action == GLFW_RELEASE && key == GLFW_KEY_UP) {
		for (Entity entity : registry.players.entities) {
			...
			Motion& motion = registry.motions.get(entity);
			Sliding sliding;
			sliding.velocity = motion.velocity;
			if (!registry.slidings.has(entity)) {
				registry.slidings.emplace(entity, sliding);
			}
		}
	}
	...
	if ((action == GLFW_REPEAT || action == GLFW_PRESS) && key == GLFW_KEY_DOWN) {
		for (Entity entity : registry.players.entities) {
			...
			Motion& motion = registry.motions.get(entity);
			float angle = motion.angle;
			motion.velocity[1] = sin(angle) * velocity/2;
			motion.velocity[0] = cos(angle) * velocity/2;
			registry.slidings.remove(entity);
		}
	}
	...
	if ((action == GLFW_REPEAT || action == GLFW_PRESS) && key == GLFW_KEY_LEFT) {
		for (Entity entity : registry.players.entities) {
			...
			Motion& motion = registry.motions.get(entity);
			float angle = motion.angle;
			motion.velocity[0] = sin(angle) * -velocity / 2;
			motion.velocity[1] = cos(angle) * velocity / 2;
			registry.slidings.remove(entity);
		}
	}
